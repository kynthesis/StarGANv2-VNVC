...
class GenResBlk(nn.Module):
    def __init__(
        ...
        if self.needSkipConvolution:
            self.conv1x1 = nn.Conv2d(dim_in, dim_out, 1, 1, 0, bias=False)
        self.toRGB = RGBBlock(style_dim, dim_out, upsample, 1)
...
class GeneratorModulated(nn.Module):
    def __init__(
        self, dim_in=48, style_dim=48, max_conv_dim=48 * 8, w_hpf=1, F0_channel=0
    ):
        super().__init__()
        ...
        for lid in range(repeat_num):
            ...
            self.decode.insert(
                0, GenResBlk(dim_out, dim_in, style_dim, upsample=_downtype)
            )  # stack-like
            dim_in = dim_out
        ...
        if F0_channel != 0:
            self.decode.insert(
                0, GenResBlk(dim_out + int(F0_channel / 2), dim_out, style_dim)
            )
...
def build_model(args, F0_model, ASR_model, modulated=False):

    if modulated:
        generator = GeneratorModulated(
            args.dim_in,
            args.style_dim,
            args.max_conv_dim,
            w_hpf=args.w_hpf,
